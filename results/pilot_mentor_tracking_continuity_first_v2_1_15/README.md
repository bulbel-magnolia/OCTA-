...
[15 帧血管椭圆图](v2_1_sampled_15_ellipses.png)补充显示用于血管源区的椭圆：青色为直接定位（11 帧），橙色为通过短缺口补全的位置（2 帧），排除帧不绘制有效源区（2 帧）。椭圆使用同一结果表的 X4 中心、X1 宽度和 128 μm 轴向管径；没有重新定位或修改结果。补全只估计位置，不生成或替换该 B-scan 的原始信号。

![15 帧血管椭圆定位](v2_1_sampled_15_ellipses.png)
